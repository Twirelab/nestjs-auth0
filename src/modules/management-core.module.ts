...
import {
  ManagementAsyncOptions,
  ManagementOptionsFactory,
} from "../auth0.options";
...
export class ManagementCoreModule {
  ...
  static forRootAsync(options: ManagementAsyncOptions): DynamicModule {
    const provider: Provider = {
      ...
      provide: MANG_CLIENT,
      useFactory: (
        authOptions: ManagementClient.ManagementClientOptionsWithClientCredentials
      ) => getManagementClient(authOptions),
    };
  ...
  private static createAsyncOptionsProvider(
    options: ManagementAsyncOptions
  ): Provider {
    ...
    if (inject.length === 0) {
      throw new Error(
        "Invalid configuration for the management module: A valid injection token is required"
      );
    }
    return {
      inject,
      provide: MANG_MODULE,
      useFactory: (optionsFactory: ManagementOptionsFactory) =>
